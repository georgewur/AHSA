...
The two examples of pressure gradients the blue and red line will result in a flow down ($-q$) and up ($q$). The numerical example data is shown below:
...
::: answer
$$
q = -K(\psi) \left ( \frac{\psi_z-\psi_{z-\Delta z}}{\Delta z} +1\right )\\
...
Below the code for generating stationary profiles.
...
for (i in 2:length(psi)){
    ...
    psi[i] = psi[i-1] - dz * (q/soil$k.fun(psi[i-1])+1)
