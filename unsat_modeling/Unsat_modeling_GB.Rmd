---
title: "Impact uncertainty vG parameters on recharge"
author: "G. Bier"
date: "October 2023"
...
For the simulation of flow in the unsaturated zone, the $k(\psi)$ is required and $\theta(\psi)$ relation will be used to determine the moisture content in the profile.

The relations between moisture content and pressure and hydraulic conductivity and pressure are:

$$
\theta(\psi) = \theta_r +\frac{\theta_s - \theta_r}{[1+(|\alpha\psi|)^n]^{m}}\\
S = [\left ( 1+|\alpha \psi|\right )^n]^{-m}   \\
k(\psi) = k_{sat} S^{\lambda}(1-(1-S^{1/m})^m)^2
$$


```{r}
pF.pnt = seq(4.2,-5,length.out = 300) #generate 300 points in the pF (-log(psi)) range of 0 and 4.2
psi = -10^pF.pnt

generate.vG.curves = function(type = string,n,alpha,theta.sat,theta.res,ksat,lambda)
{
  m = 1-1/n    ##m is just a helper parameter
  S = (1+abs(alpha*psi)^n)^(-m) ## S is also some intermediate result
  theta.un.table = theta.res+(theta.sat-theta.res)*S ##here a table is setup for the theta(psi)-relation
  K.un.table = ksat*S^lambda*(1-(1-S^(1/m))^m)^2 ##here the table holds the K(theta) data           
  K.un.table[K.un.table < 1e-5] = 1e-5
  C.un.table = (theta.sat-theta.res)*m*n*alpha*abs(alpha*psi)^(n-1)*
    (1+abs(alpha*psi)^n)^(-m-1)
  #below functions are created from the calculated data points
  k.fun <<- approxfun(psi,K.un.table,rule=2)
  theta.fun <<- approxfun(psi,theta.un.table,rule=2)
  c.fun <<- approxfun(psi,C.un.table,rule=2)
  
}

#just a view vG values for some soils.
##########################VAN GENUCHTEN CURVES##############################################
# soil type|theta.res|theta.sat|n     |alpha  |Ksat |lambda |soil name
#------------------------------------------------------------------------
# B1       |0.02     |0.43     |1.801 |0.0234 |23.41|-0.000 |fine/moderate fine sand        
# B5       |0.01     |0.36     |1.933 |0.0452 |52.91|-0.359 |coarse sand
# B8       |0.01     |0.43     |1.288 |0.0099 |2.36 |-2.244 |loamy sand
# B12      |0.01     |0.54     |1.094 |0.0239 |5.37 |-5.681 |heavy clay

generate.vG.curves("B01",1.801,0.0234,0.43,0.02,23.41,-0.000)#data of B1


#two plot function for inspecting the k(psi) and theta(psi) relations of the current soil
plot.kfun = function(soiltype = string)
{
  plot(k.fun(psi),pF.pnt, type = "l", lwd = 2, col = "blue",
       main = paste("Hydraulic conductivity as function of pF of",soiltype),
       xlab = "k(psi), (cm/d)", ylab = "|log(psi)|")
  grid()
}

plot.thetafun = function(soiltype = string)
{
  plot(theta.fun(psi),pF.pnt, type="l", lwd = 2, col = "red",
       main = paste("Moisture content as function of psi (-) of",as.character(soiltype)) ,
       xlab = "moisture content (fraction)",
       ylab = "|log(psi)|")
  grid()
}
...
Ik zag in O15 een max k.sat staan van 178.593 maar die komt niet terug in de grafiek!

Dus maar even vergelijken met de data uit het rapport(water retentie karakteristieken.....2018). Hmmm

Dat is op log(k.sat)-log(psi) schaal gedaan, dus hier ook maar even.

```{r}
plot.logkfun = function(soiltype = string)
{
  log.kfun = log10(k.fun(psi))
  log.pFpnt = log10(pF.pnt)
  plot(log.pFpnt, type = "l", lwd = 2, col = "blue",
       main = paste("Hydraulic conductivity as function of pF of",soiltype),
       xlab = "k(psi), (cm/d)", ylab = "|log(psi)|")
  grid()
}
...
ksat = 22.32
lambda = 2.868
generate.vG.curves(soiltype,n,alpha,theta.sat,theta.res,ksat,lambda)


## domain of the model
top = 0.0  # at the surface
bottom = -2000#-200 # so 200 cm below surface!!
...
set.discretisation(UnsatStat,nodes,"FE") #setting to FEquartic slows transient simulations way too much
...
drn.level = -80 # so 80 cm below surface level
...
### checking the data from staring reeks

```{r}
vG = read.csv("Staring_2018_vanGenuchten.csv", header = TRUE)
summary(vG)

Soiltypes = unique(vG[,1])

B05 = vG[which(vG$BS == "B05"),]
summary(B05)
hist(B05$alpha)

O05 = vG[which(vG$BS == "O05"),]
summary(O05)
hist(O05$alpha)

B01 =vG[which(vG$BS == "B01"),]
summary(B01)
hist(B01$alpha)


plot.vG.curves = function(soil = data.frame)
{
  old.par = par(no.readonly=TRUE) #to save current plot settings and stuff
  #par(mfcol = c(2,1))
  layout = matrix(c(1,2),ncol = 1, byrow = TRUE)
  kleuren = rainbow(length(soil[,1]))
  generate.vG.curves(soil,soil$n[1],soil$alpha[1],soil$theta.sat[1],soil$theta.res[1],soil$ksat[1],soil$lambda[1])
  k.range = c(0,max(soil$ksat))
  ##generate the k.fun plot
  plot(k.fun(psi),pF.pnt, type = "l", lwd = 1, xlim = k.range, col = kleuren[1],
       main = paste("Soil type :",soil$BS[1]), xlab = "k (cm/d)", ylab = "|psi| (cm)")
  for (i in 2:length(soil[,1]))
  {
    generate.vG.curves(soil,soil$n[i],soil$alpha[i],soil$theta.sat[i],soil$theta.res[i],soil$ksat[i],soil$lambda[i])  
    lines(k.fun(psi),pF.pnt,col = kleuren[i])
  }
  grid()
  ##generate the theta.fun plot
  generate.vG.curves(soil,soil$n[1],soil$alpha[1],soil$theta.sat[1],soil$theta.res[1],soil$ksat[1],soil$lambda[1])
  plot(theta.fun(psi),pF.pnt, type="l", lwd = 2, col = "red",
       main = paste("Moisture content as function of psi (-)"),
       xlab = "moisture content (fraction)",
       ylab = "|log(psi)|")
  for (i in 2:length(soil[,1]))
  {
    generate.vG.curves(soil,soil$n[i],soil$alpha[i],soil$theta.sat[i],soil$theta.res[i],soil$ksat[i],soil$lambda[i])  
    lines(theta.fun(psi),pF.pnt,col = kleuren[i])
  }
  grid()
  par = old.par #to reset the plot settings to the original settings
}
...
### setting up the LSA basics

Idea is to see the effect of the derivative of a certain vG parameter w.r.t. a result is large or small.

$\frac{\partial O}{\partial P_i}$

The result is in this case the output of the model:  

*  water table height  
*  recharge at the bottom  
*  actual evapotranspiration (through roots)  
*  moisture content  

Let's start with recharge at the bottom.
So except for one parameter all other parameters and inputs stay the same.

```{r}
ppt = 0.2
epot = 0.1

P.mean = colMeans(B01[,3:9])
# P.names = names(P.mean)
# P.mean = unname(P.mean)
# colnames(P.mean) = P.names
P.sd = apply(B01[,3:9], MARGIN = 2, FUN = sd)
# P.sd = data.frame(P.sd,colnames = names(P.sd))

#take lambda and calculate a relative eps based on 1% of the sd of this parameter
eps.lambda = 0.01 * P.sd["lambda"]
eps.ksat = 0.01 * P.sd["ksat"]
##B01 soil
generate.vG.curves("B01",1.801,0.0234,0.43,0.02,23.41,-0.000)#data of B1
plot.kfun("B01")
plot.thetafun("B01")
...
plot(UnsatStat,fluxplot = T)
solve.steps(UnsatStat,verboselevel = 1)
df.state = dataframe.states(UnsatStat)
df.balance = dataframe.balance(UnsatStat)
df.bound = dataframe.boundaries(UnsatStat)
df.extern = dataframe.externalfluxes(UnsatStat)

generate.vG.curves("B01",1.801+eps.ksat,0.0234,0.43,0.02,23.41,-0.000)
plot.kfun("B01")
plot.thetafun("B01")
do.initialize(UnsatStat,drn.psi.equi)
plot(UnsatStat,fluxplot = T)
solve.steps(UnsatStat,verboselevel = 1)
df.state.ksat = dataframe.states(UnsatStat)
df.balance.ksat = dataframe.balance(UnsatStat)
df.bound.ksat = dataframe.boundaries(UnsatStat)
df.extern.ksat = dataframe.externalfluxes(UnsatStat)

#generate.vG.curves(P.mean["n"],P.mean["alhpa"],P.mean["theta.sat"],P.mean["theta.res"],P.mean["ksat"],P.mean["lambda"])
...
Since we are calculating $\psi$ as the state of the model, we are going to use the `theta.fun(psi)` function to determine the amount of moisture content based on the current pressure head ($\psi$)
...
{
  storage = (theta.fun(psi.old(z)) - theta.fun(state))/delta.t
}
```
...
#This base run is based on a B01 Soil
generate.vG.curves("B01",1.801,0.0234,0.43,0.02,23.41,-0.000)#data of B1
...
#precip = c(0.01,0.02,0.03,0.0,0.4,0.0,0.0,0.15,0.015,0.2)
#evapot = c(0.0,0.03,0.08,0.1,0.125,0.09,0.2,0.25,0.05,0.12)
...
while(current.time <= end.time)
{
  ...
  solve.steps(UnsatTrans)#,verboselevel = 1)
...
#### LSA on transient simulations

Calculate the LSA by:

$$
 VAR(O_i)= \sum \left [ \frac{1}{\epsilon_{P_j}} \left( 1/T\sum^T_{t=1} \left( O_i(t)^{P_j}-O_i(t)^{P_j+\epsilon_{P_j}}\right ) \right ) \sigma P_j \right ]^2
$$
So $O_i$ is for example the recharge and $P_j$ is $\alpha$.
The term between the large parenthesis (curved brackets) is in this case simply the time average of $O_i(t)^{P_j}-O_i(t)^{P_j+\epsilon_{P_j}}$ in this example the recharge and this can be easily calculated by calculating the mean difference between `balance.base$recharge - balance.res$recharge`


We will take a relative fraction (1%) of the parameter value being the $\epsilon$.
Sometimes the $\lambda$ value seems to have a value of 0.000 or even -0.000. In this case a fixed of for example $\epsilon_\lambda = 0.001$ could be used.

```{r}
##LSA on kSat parameter
ksat.eps = 0.01 * base.Ksat #calculate the epsilon
#generate new curves based on Ksat + ksat.eps
generate.vG.curves("B01",base.n,base.alpha,base.thetasat,base.thetares,base.Ksat+ksat.eps, base.lambda)
##
##Run the transient model with these parameters
## this chunck is the same as before
begin.time  = 1
current.time = begin.time
end.time = length(meteo$date)
delta.t = 1 #day, i.e. the time stepping
...
#set the initial state (psi) distribution
do.initialize(UnsatTrans, init = drn.psi.equi)
...
##calculating the sensitivities for the time series
dRecharge.dKsat.mean = mean(balance.base$Recharge - balance.res$Recharge)/ksat.eps
dRecharge.dKsat.absmean = mean(abs(balance.base$Recharge - balance.res$Recharge))/ksat.eps
dRecharge.dKsat.sumvar = sum((balance.base$Recharge - balance.res$Recharge)^2)/ksat.eps
dRecharge.dKsat.VAR = 1/length(balance.base$Recharge) * sum((balance.base$Recharge - balance.res$Recharge)^2)/ksat.eps ##sensitivity expressed as variance/eps

dTransp.dKsat.mean = mean(balance.base$Eact - balance.res$Eact)/ksat.eps
dTransp.dKsat.absmean = mean(abs(balance.base$Eact - balance.res$Eact))/ksat.eps
dTransp.dKsat.sumvar = sum((balance.base$Eact - balance.res$Eact)^2)/ksat.eps  ## sensitivity expressed as total variance sum((delta_Output)^2)/eps
dTransp.dKsat.VAR = 1/length(balance.base$Recharge) * sum((balance.base$Eact - balance.res$Eact)^2)/ksat.eps

###calculating the water table from the state.res
wtable.fun = function(x)
{
  psi.fun = approxfun(x,nodes)
  return(psi.fun(0))
}
wtable.base = apply(X=state.base, MARGIN = 1, FUN = function(x){wtable.fun(x)})
wtable.res = apply(X=state.res,MARGIN = 1, FUN = function(x){wtable.fun(x)})

dwtable.dKsat.mean = mean((wtable.base - wtable.res),na.rm = TRUE)/ksat.eps
dwtable.dKsat.absmean = mean(abs(wtable.base - wtable.res),na.rm = TRUE)/ksat.eps
dwtable.dKsat.sumvar = sum((wtable.base - wtable.res)^2, na.rm = TRUE)/ksat.eps
dwtable.dKsat.VAR = 1/length(balance.base$Recharge) * sum((wtable.base - wtable.res)^2, na.rm = TRUE)/ksat.eps

###reset the van Genuchten parameters
generate.vG.curves("B01",base.n,base.alpha,base.thetasat,base.thetares,base.Ksat, base.lambda)
...
Next we use IJPLOT package from Paul Torfs (old colleague of mine) to look at the moisture, pressure and root uptake distribution in the model in time. These are thus time series
...
add_selectnum(ijp=flux.plot,name = "time step",maxnum = nr.time.steps)
for (i in 1:nr.time.steps)
{
  add_line(flux.plot,x = nodes, y = theta.fun(state.res[i,]), selectnum = c("time step", i), color = color.set[1])
...
theta.range = c(theta.fun(psi.min),theta.fun(psi.max))
...
flux.legend = new_ijplot(name = "flux.legend",yfixed = TRUE, xfixed = TRUE,fxleft = 0.90,fybottom = 0.8,
...
nr.time.steps = length(moist.res[,1])
add_selectnum(mip=psi.plot,name = "time step",maxnum = nr.time.steps)
for (i in 1:nr.time.steps)
{
