...
The first term on the right can be expressed as the "Lateral inflow slope"; $S_i=2\frac{Q}{gA^2}I$
...
NOTE that we are considering a constant Side Inflow $I$ which does **not** depend on the water depth. This means that we only consider nearby ditches discharging into the open water course and not considering local drainage underneath the open water course.
...
plot(s,Zb.fun(s), type = "l", lwd = 3, col = "brown", 
     main = "Bottom elevation Z", xlab = 'Distance to control stucture (e.g. weir)',
     ylab = 'Altitude (m)')
grid()
```
...
In lowland areas like the Netherlands, most open water courses are managed by weirs. They may have different shapes and sizes. Here we will only consider rectangular weirs. Below a plot indicating the different dimensions of the weir within the trapezoidal profile of the open water course.

```{r echo=FALSE}
weir.b = 6.0  #m width of the weir 
weir.d = 1 #m crest above river bottom
# draw weir in cross section
coords_profile = matrix(c(6.5,0,6,11,17,10.5, 3,3,0,0,3,3),nrow = 6,byrow = FALSE)
coords_weir = matrix(c(6.5,6.5,10.5,10.5, 3,2,2,3), nrow = 4, byrow = FALSE)
plot(coords_profile,type = "l",lwd = 3, col = "brown",xlab = "",ylab = "", axes = FALSE,
     main = "Dimensions of the profile and weir")
lines(coords_weir,type = "l", lwd = 3, col = "blue")
grid(nx = 10, ny = 10)
arrows(8.5,0,8.5,2.0,length = 0.1, angle = 30,code = 3,col="blue",lwd =2)
text(8.8,1,"crest height",pos=3,srt=90,col="blue")
arrows(6.5,2.0,10.5,2.0,length = 0.1, angle = 30, code = 3, col = "blue",lwd =2)
text(8.5,2.0,"weir width",pos=3,col="blue")
box()
```
...
### Determining the equilibrium water level for a given discharge

To calculate the equilibrium water level for a certain discharge (instead of using the graph above), one can use an optimization/root finding routine within R. There are quite a few flavours available.\
Here we simply use the `uniroot()`

```{r}
##finding the equilibrium water depth for a given discharge
a.equilibrium = uniroot(function(a) Qequi.fun(a)-Qup,c(0.1,2))$root
print(paste("the equilibrium water level is : ",round(a.equilibrium,3), "for a discharge of ",round(Qup,3)))
```
...
The calculation of the water level in the river can be approximated by an explicit scheme. So:
...
To determine the $\frac{\Delta a}{\Delta s}$, the code starts at the downstream end, where (for now) the water level is prescribed. The water depth at position $\Delta s$ upstream, $a[s + \Delta s]$, is based on $\,S_f(a[s])$ and $\,Fr^2(a[s])$. This can be accomplished using a loop, starting at the downstream end and moving upstream with a $\Delta s$ spatial step;

`For all a[s] till the before last location, which is a[s=L] do:`

-   `Calculate S_f(a[s]) and Fr(a[s])`
-   `Calculate da/ds with S_0, S_f(a[s]) and Fr(a[s])`
-   `Calculate a[s + ds] = a[s] + da/ds * ds$`
...
Fill out the XXXX in the following chunk to calculate the backwater curve without side inflow $I$
...
In de previous assignment the downstream water level was given. Now we will calculate the water level downstream of a weir. The weir is a simple rectangular weir, see the section above, with a discharge coefficient of 0.62.

### The weir function

::: {.question style="color: red"}
Write a function to calculate the water level at the weir based on the discharge $Q$. The function should return the water level $h$. To do this the weir function to calculate the discharge need to be rearranged to calculate the head $h$ above the weir for a given discharge $Q$. This results in following:

$$
h = d + \left( \frac{Q}{\frac{2}{3}\sqrt{2g}C_D b} \right)^{2/3}
$$

Replace the XXXX and YYYY in the following chunk.
...
hweir = function(Q)
{
  return (XXXX + (Q/(2/3*sqrt(2*9.8)*Cd*YYYY))^(2/3))
}
...
Up till now the water level at the downstream end is calculated with a prescribed head. Now you will replace this with the weir function. The code is similar to the previous code chunk but now with the weir function, to replaced the prescribed downstream water level `a.bw[1]`.
...
::: {.question style="color: red"}
Calculate the backwater curve with the Froude is = 0 approach. Copy the previous chunk and replace the calculation of the gradient with the Froude is = 0 approach.

```{r}
####same chunck as above but now with Fr=0 approach
```
:::
...
```{r}
####same chunck as above but now with a weir level based on Qup
####and neglecting Fr in case of a.bwf0
# first downstream prescribed water level
a.bw[1] = hweir(Qup)
a.bwf0 = a.bw
# Discharge rate upstream Qup
