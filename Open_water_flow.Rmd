...
subtitle: "Backwater curves in WANT"
output: 
  html_notebook:
   css:  want_n.css
   number_sections: yes
...
with: $A$ = cross sectional area of the river (m$^2$), $Q$ = discharge (m$^3$/s), $u$ = flow velocity (m/s), $s$ = distance along the river (m), $t$ = time (s), $I$ = side inflow (m$^2$/s), $S_o$ = bed slope (m/m), $S_f$ = friction slope (m/m), $a$ = water level (m), $g$ = acceleration due to gravity (m/s$^2$).
...
As determined above, the $u$ stands for the velocity and is similar to : $u = \frac{Q}{A}$ . In addition the cross-sectional area is $a$ function of the water depth and the width at the water level $b$: $\frac{\partial A}{\partial a}=b(a)$.
...
When there is no side inflow, discharging ditches and local drainage is disregarded resulting in a constant discharge entering the open water course upstream and leaving it a the downstream (weir) end.
...
Implementing this in the stationary momentum equation:
...
When this number is larger than 1 water is shooting instead of flowing when this number is less than 1, much less in most practical circumstances.
...
When there is side inflo the following derivation need to be followed;
...
\frac{\partial Qu}{\partial s}=2QA^{-1} I-Q^2A^{-2} b(a)\frac{\partial a}{\partial s}\\
\text{so}\\
-Q^2A^{-2} b(a)\frac{\partial a}{\partial s}= g\;A\;\Big( S_o - S_f - \frac{\partial a}{\partial s}\Big)\\
...
The first term on the right can be expressed as the "side slope"; $S_i=2\frac{Q}{gA^2}I$

and this finally results in an equation calculated the backwater curve including the side-slope:

$$
\frac{\partial a}{\partial s}= \frac{S_o-S_f-S_i}{1-Fr^2}
$$

NOTE that we are considering a constant side inflow $I$ which does **not** depend on the water depth. This means that we only consider discharge ditches nearby into the open water course and not considering local drainage underneath the open water course.

# Geometry of the open water domain

The cross-sectional geometry of the open water course is of major importance since the hydraulic properties of $A, P$ and $R$ change w.r.t. the water level $a$. Therefor extra attention is paid to this aspect. Here we will only consider trapezoid profiles. Although simplified, these profiles already have quite a non-linear behaviour regarding $A, P$ and $R$

Below an illustration of such a trapezoid profile:

![A trapezoid profile](Trapezodial.png)

$A$ is the wetted Area of the profile, $P$ being the wetted perimeter (the length of the profile which is wet due to the water level) and $R$, being the hydraulic radius and determined by: $R=\frac{A}{P}$.
...
```{r}
S0 = 0.00015 # about 0.15 m/km for the midden Regge
...
The calculation of the water level in the river can be done by a explicit scheme. So:
...
To determine the $\frac{\Delta a}{\Delta s}$, code starts at the downstream end, where (for now) the water level is prescribed. The water depth $\Delta s$ upstream, $a[s + \Delta s]$ is based on $\,S_f(a[s]$ and $\,Fr^2(a[s])$. This can be accomplished using a loop, starting at the downstream end and moving upstream with a $\Delta s$ spatial step. For all $a[s]$ till the before last location, which is $a[s=L]$ do:
