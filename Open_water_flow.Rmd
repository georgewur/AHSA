...
Qs = rep(0,times = length(a.si))
# the first Q is the Q at the weir
Qs[1] = Qup
# loop for calculating the water levels
for (i in 1:(length(a.si)-1) )
{
  A = ow.A(a.si[i])
  Q = Qs[i] - I*-ds  ## ds is negative!! and I*ds should be subtracted for the Qs upstream
  u = Q/A
  S.i = 2*u*I/(9.8*A)
...
plot(s,h.si, type = "l",ylim = c(0,max(h.bw)),xlab = "S (m)", ylab = "Water level (m)",
     main = paste("Bw curve,Si and weir,S0",S0, "Qupstream",Qup),col = "blue",lwd = 3)
lines(s,Zb.fun(s), col = "brown", lwd = 3)
grid()
##plot the water level at equilibirum
lines(s,a.equilibrium + Zb.fun(s), col = "red", lty = 2, lwd = 3)
...
plot(s,h.si-h.sif0, type = "l",xlab = "S (m)", ylab = "Difference in water level (m)",
     main = paste("Difference bwsi and bwsifr=0,S0",S0, "Qupstream",Qup),col = "blue",lwd = 3)
grid()
